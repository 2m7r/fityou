      ...
      <div class="calendar-body">
        <div class="calendar-grid">
          ...
          <div v-for="(date, index) in calendarDates" :key="index" class="calendar-date">
            <div v-if="date !== ''" :class="['calendar-date-item', getDayStatus(date)]">
              {{ date }}
            </div>
          </div>
        </div>
...
const getDayStatus = (date) => {
  ...
  const month = currentMonth.value < 10 ? `0${currentMonth.value}` : currentMonth.value; // 월을 2자리로 맞추기
  ...
  .calendar-date-item.workout {
    background-color: #b1c2d1; /* 운동일기 색상 */
  }
  
  .calendar-date-item.diet {
    background-color: #a4b491; /* 식단일기 색상 */
  }
  
  .calendar-date-item.both {
    background-color: #4f8d67; /* 운동과 식단 둘 다 작성한 경우 */
  }
